...
//Create a database connection
   ...
   /* $conn = new mysqli(DB_SERVER, DB_USER, DB_PASS,DB_NAME);
    ...
    if(isset($_GET['dropdown'])) {
		...
		if((strlen($_GET['searchstr']) !=0)&&isset($_GET['searchstr'])) 
		{
		$totalstring = $_GET['searchstr'];}
...
$tags=array();
		$yearrfilter = "";
		$tagfilter = "";
		if(isset($_POST['filteryear']) || isset($_POST['filtertag'])) {
			$flag1 =0;
$scover = 0;
		if(isset($_GET['searchcover'])) {
			$scover = $_GET['searchcover'];
			//echo "<br/>Search Cover:{$scover}";
		}
...
$x = 0;
foreach($_POST['filteryear'] as $selected){
//echo "YEAR:".$selected."</br>";
$y=$x+1;
$year[]=substr($selected,0,4);
$year[]=substr($selected,4);
...
//echo "string:".$str;
$token= explode( "," , $str);
 $years = $_POST['filteryear']; 
    $Nyears = count($years); 
    
   
    $tags = $_POST['filtertag'];
    $Ntags = count($tags); 

    $years_between = "";
    for($i=0; $i < $Nyears; $i++)
                {
                    $single_year = explode( "-" , $years[$i] );
                    
                   // echo "<br><br>^^^^^^^^1::".$single_year[0];
                    //echo "<br>^^^^^^^^2::".$single_year[1];
                    
                    $years_between .= "(year BETWEEN ".$single_year[0]." and ".$single_year[1].")";
                    $years_between .= " or ";
 ...
 for($i=0; $i < count($token); $i++)
                {
                    $single_tag = explode( "," , $str );
                    
                   // echo "<br><br>^^^^^^^^1::".$single_year[0];
                    //echo "<br>^^^^^^^^2::".$single_year[1];
                    
                    $tag_b .= "(TrackArtistName =".$single_tag[0]." and ".$single_tag[1].")";
                    $tab_b .= " or ";
                    
                 //echo ($i+1)."] ".$years[$i]."<br>";
           }
$yearfilter= chop($yearfilter, " OR");
				$yearfilter.=")";
//echo $yearfilter;
//echo $years_between;
          $years_between = substr( $years_between , 0, -1 );
           $years_between = substr( $years_between , 0, -1 );
           $years_between = substr( $years_between , 0, -1 );
          // echo $years_between;
}
...
{
// Loop to store and display values of individual checked checkbox.
foreach($_POST['filtertag'] as $selected){
...
if($flag==1){
				...
				foreach($_POST['filtertag'] as $tag ){
					$tagfilter.="	TagName LIKE \"%{$tag}%\" OR";		
				}
				$tagfilter= chop($tagfilter, " OR");
				$tagfilter.=")";
				//echo $tagfilter;
//echo $tags[1];
}
	...
	<tr>
		...
		<ul class ="sub">
			...
			<?php
					...
					global $years_between;
					global $tagfilter;
					//echo "ye lo years".$years_between.$tagfilter;
...
//	echo $temp;
   $createtable1 = "CREATE TEMPORARY TABLE table3 (SELECT t1.TrackID, (SELECT COUNT(t2.TagName) FROM {$tagtable} AS t2 WHERE t1.TrackID = t2.TrackID) AS totaltrack, (SELECT t3.TrackTitle FROM {$tracktable} as t3 WHERE t1.TrackID = t3.TrackID) AS title, (SELECT t3.TrackArtistName FROM {$tracktable} as t3 WHERE t1.TrackID = t3.TrackID) AS artistName, (SELECT t3.Year FROM {$tracktable} AS t3 WHERE t1.TrackID=t3.TrackID) AS year FROM initial5 as t1 ORDER BY totaltrack DESC);";
	...
	//echo $createtable1;
	$q0="SELECT * FROM table3 where {$years_between} LIMIT 0, 15;";
	//echo"ye nyi query:".$q0;
	$result2 = mysql_query($q0, $connect);
		  
		  
		}
		  			$q5="SELECT MIN(year) AS minyear FROM table3 where {$years_between};";
		  			//echo $q5;
					$yquery = mysql_query($q5, $connect);
					
					if($resultyear = mysql_fetch_array($yquery)){
						echo "filter me hu andar";
						
						$minyear = $resultyear["minyear"];
						echo "MIN YEAR: {$minyear}";
						$q6="SELECT MAX(year) AS max_year FROM table3 where {$years_between};";
					...
					$tagtablequery = "CREATE TEMPORARY TABLE taginitial1(SELECT t1.TagName, (SELECT COUNT(t2.TagName) FROM {$tagtable} as t2 WHERE t3.TrackID=t2.TrackID) AS count FROM {$tagtable} as t1, initial5 as t3 WHERE t3.TrackID=t1.TrackID);";
					//echo $tagtablequery;
					$tagint = mysql_query($tagtablequery, $connect);
					
					
					$tagselectquery = "SELECT DISTINCT TagName FROM taginitial1 {$tagfilter} ORDER BY count DESC LIMIT 0, 5;";
					//echo "tag h yha".$tagselectquery;
					if($tagint)
					{
